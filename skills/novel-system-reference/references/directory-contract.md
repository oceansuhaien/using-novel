...
在需要返工已落盘正文时，可在单本书工作区内引入**主线**与**改稿分支**语义。主线仍代表当前正式事实；改稿分支用于隔离返工过程。返工推进单位不是单章，而是**受影响文件包**。
...
- `chapters/`：章节级观察、摘录、证据、临时笔记；正文执行审计默认也落在这里，优先写入 `chapters/notes/`。
- `inbox/`：原始灵感、备选方案、未拍板问题、尚未确认的材料。
- 主线：当前正式事实工作线。
- 改稿分支：返工已落盘正文时使用的隔离工作线。
- 受影响文件包：返工推进的最小确认单位，可横跨章节、人物卡、`plot/`、`context.md` 与 `summary.md` 位点。
...
- 完整正文执行审计不写进 `context.md` / `summary.md` 主体；只有稳定结论才允许摘要式引用。
- 未拍板方案进 `inbox/`，不要写成定稿。
