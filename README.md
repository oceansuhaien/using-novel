...
| `novel-plot-weaver` | 用于**筹备期**把中文网文剧情灵感整理成可推进的主线、暗线、伏笔、反转、阶段目标、卷钩子和剧情节点。适用于尚未动笔正文的阶段，做剧情结构规划、埋伏笔、设计卷钩子、梳理推进逻辑、处理节奏断裂，并在用户确认后把稳定结论写回 `plot/` 等架构模块。**不**用于场景正文阶段——场景正文由 novel-scene-* 系列基于角色自主推理产出，plot 节点只作为筹备输入。 |
